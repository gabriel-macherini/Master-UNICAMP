//#### patchnotes
// alteração de maio:
// 6+meio+6 para 4+meio+4, redução de n de z
// setado visual em composite para poder avaliar qual melhores cortes
// add waitForUser("quer deletar algum núcleo?"); na linha 83 pra remover roi de núcleo zoado

// O objetivo desse código é separar os loci de bclaf1 (por findmáxima de p<180.000) e fazer circúlos dessa área.
...
for (f=0; f<filelist.length; f++) { //loop pra cada arquivo da lista de arquivo
	...
	// dar duplicate no slices m-6 até m+6 (alterado em maio para 4+meio+4)
	...
	saveAs("Results", input_path + "\\results\\" + title + "_findmaxima"+".csv");
	...
	roiManager("Deselect");
	roiManager("Measure");
	saveAs("Results", input_path + "\\results\\" + title + "_loci"+".csv");
